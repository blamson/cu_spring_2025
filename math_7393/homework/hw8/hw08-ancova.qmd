...
```{r}
Hawks %>%
    ggplot(aes(x=Wing, y=Weight, color=Species)) +
    geom_point()
```
...
As for relationships between wing length and body weight, SS hawks seem to have a pretty small relationship between there. RT hawks seem to have a stronger relationship between the two variables. RT hawks are by far the largest of the three species and with (potentially) the strongest relationship between the two variables. It's also worth noting that there are a lot of extreme points in the RT species, so we'll see how that affects the models.
