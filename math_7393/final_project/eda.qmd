...
if (file.exists(raw_data)) {
    df <- haven::read_sas("data/spm_pu_2022.sas7bdat")
} else {
...
df_trim <- df %>%
    select(c(
        , "SPM_Poor" # poverty classification
        , "SPM_PovThreshold" # SPM unit poverty threshold
        ...
        , "Moop_other" # medical out of pocket expenses
    )) %>%
    rename_all(tolower)
```
...
```{r}
df_trim$spm_poor %>% hist()
```

```{r}
df_trim$spm_poor %>% table()
```
